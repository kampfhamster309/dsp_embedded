/**
 * @file test_dsp_psk.c
 * @brief Host-native unit tests for the dsp_psk component (DSP-204).
 *
 * Tests cover:
 *   - Lifecycle: init/deinit idempotence, double-call safety
 *   - dsp_psk_set(): NULL/zero-length/too-long/too-short argument rejection,
 *     valid call succeeds, is_configured() reflects state
 *   - dsp_psk_get_identity() / dsp_psk_get_key(): NULL out_len returns NULL,
 *     returns NULL before set, returns correct pointer/length/content after
 *     set, returns NULL after deinit
 *   - Compile-time guard (ESP_PLATFORM absent in host builds)
 *
 * dsp_psk_apply() is ESP_PLATFORM-only and is not tested here.
 */
...
static const uint8_t s_key[] = {                                         /* 32 B = max */
...
/* Helper: ensure clean state before state-sensitive tests */
static void reset_psk(void)
{
    dsp_psk_deinit();
}
...
{
    TEST_ASSERT_EQUAL(ESP_OK, dsp_psk_init());
    TEST_ASSERT_EQUAL(ESP_OK, dsp_psk_init());  /* idempotent */
    dsp_psk_deinit();
}
...
{
    /* Fake oversized identity – pointer doesn't need to be valid since
     * the length check fires before any dereference. */
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG,
...
{
    /* Key shorter than DSP_PSK_MIN_KEY_LEN (16 B) is rejected */
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG,
                      dsp_psk_set(s_id, sizeof(s_id),
...
/* -------------------------------------------------------------------------
 * dsp_psk_get_identity() / dsp_psk_get_key() tests
 * ------------------------------------------------------------------------- */
